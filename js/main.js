...
/* Pointer + scroll parallax across the site (hero sigil, HUD, section
   vectors). Pure enhancement; disabled under reduced motion / coarse
   pointers. Booted immediately so the hero sigil has depth from first paint. */
import('./parallax.js').then(({ initParallax }) => initParallax()).catch(() => {});
